...
function renderQuotaTable(
  ...
  lines.push(
    pc.dim(
      "  ┌" +
        "─".repeat(modelColWidth) +
        "┬" +
        "─".repeat(quotaColWidth) +
        "┬" +
        "─".repeat(resetColWidth) +
        "┐"
    )
  )
  lines.push(
  ...
  // Sort models by name
  const sortedModels = [...quota.models].sort((a, b) =>
    a.name.localeCompare(b.name)
  )

  for (const model of sortedModels) {
    const displayName =
      model.name.length > modelColWidth - 3
  ...
  if (isLoading) {
    ...
    lines.push(
      pc.dim(
        `  ⟳ Auto-refresh in ${countdown}s | Press 'r' to reload | Press 'q' to exit`
      )
    )
  }
...
function setupKeyHandler(onReload: () => void, onExit: () => void): () => void {
...
${pc.dim("(The account was likely added using --manual mode)")}`,
    ...
    const refreshInterval = Math.max(
      10,
      Number.parseInt(args.interval, 10) || 30
    )

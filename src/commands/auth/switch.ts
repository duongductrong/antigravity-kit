  ...
  ${pc.cyan("antigravity-kit auth add")}`,
		...
		if (isRunning) {
			const shouldContinue = await p.confirm({
				message: `${pc.yellow("Antigravity is currently running.")} Close it to continue?`,
				initialValue: true,
			});
			...
			spinner.start("Waiting for Antigravity to close...");
		...
		if (shouldLaunch) {
			try {
				...
				p.outro(
					`${pc.green("✔")} Switched to ${pc.cyan(selectedProfile.email)} and launched Antigravity`,
				);
			} catch (error) {
				p.outro(
